import {splitColors} from '@bhsd/common/dist/index.cjs'; // eslint-disable-line n/no-missing-import
...
const findColors = (doc: TextDocument, tree: AstNodes): ColorInformation[] => {
	...
	return childNodes.flatMap(child => {
		...
		return splitColors(child.data).filter(([,,, isColor]) => isColor)
			.map(([s, from, to]): ColorInformation => {
